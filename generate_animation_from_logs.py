...
def agent_log_parser(line) -> dict:
    ...
    # parse agent info
    agent_pos = re.search(r"HP:\s?(\d+) node:(\d+) dir:(\d)", line)
...
def generate_picture(
    ...
    for i, line in enumerate(lines):
        ...
        total_reward += int(reward.split(":")[-1])
        text_head = f"#{idx_step:2d}/{max_step} Current reward: {total_reward}"
        if i == max_step:
            # get episode rewards from log
            text_head += ""  # line[:-1]
        legend_text = [""] * 10 + [text_head]
        # set color map for agents and waypoints
        col_map = ["gold"] * len(map_info.n_info)
        for agent in agents:
            legend_text += [
                "{}_{} HP:{} node:{} dir:{}".format(
                    agent["team"],
                    agent["id"],
                    agent["HP"],
                    agent["node"],
        ...
        plt.legend(
            ...
            bbox_to_anchor=(0.07, 1.01, 0.83, 0.1),
            loc="lower left",
            prop={"size": 8},
            mode="expand",
            borderaxespad=0.0,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--HP_red", type=int, default=20)
    parser.add_argument("--TR_red", type=int, default=5)
    parser.add_argument("--HP_blue", type=int, default=20)
