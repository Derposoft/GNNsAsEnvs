...
def get_step_reward(r_engages_b, b_engages_r, overlay, **rewards):
    step_reward = 0
    if r_engages_b:
    ...
    if overlay:
        step_reward += rewards["reward_R_team"] if "reward_R_team" in rewards.keys() else REWARDS["reward_R_team"]
    return step_reward
